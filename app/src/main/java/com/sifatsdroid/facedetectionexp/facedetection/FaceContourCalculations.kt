...
class FaceContourCalculations(overlay: GraphicOverlay, private val firebaseVisionFace: FirebaseVisionFace?) :
    ...
    override fun draw(canvas: Canvas) {
        ...
        val bottomMostPoint= contour.points.minBy { it.y }
        val topMostPoint  = contour.points.maxBy { it.y }

        if (bottomMostPoint != null && topMostPoint!=null) {
            canvas.drawLine(bottomMostPoint.x,bottomMostPoint.y,topMostPoint.x,topMostPoint.y,boxPaint)
        }

        val leftMostPoint= contour.points.minBy { it.x }
        val rightMostPoint  = contour.points.maxBy { it.y }
        if (leftMostPoint != null && rightMostPoint!=null) {
            canvas.drawLine(leftMostPoint.x,leftMostPoint.y,rightMostPoint.x,rightMostPoint.y,boxPaint)
        }
